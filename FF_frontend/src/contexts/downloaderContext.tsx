...
export function DownloaderWrapper(props: React.PropsWithChildren<{}>) {
  ...
  const downloadDataset = useCallback(async (datasetId: string) => {
    ...
    const promises: { _id: string; response: Promise<Response> }[] = [];
    ...
    try {
      // Dynamically load client-zip only on the client-side
      const clientZip = await import('client-zip');
      const blob = await clientZip.downloadZip(transformed).blob();
      ...
      console.error('Error loading client-zip:', error);
    }
  }, []);

  return (
    <DownloaderContext.Provider value={ { downloadDataset } }>
      { props.children }
